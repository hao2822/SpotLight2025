...
namespace ChocDino.UIFX
{
	...
	public class FilterStackTextMeshPro : UIBehaviour
	{
					...
					if (meshInfo.vertexCount > 0)
					{
						...
						else
						{
							...
							
							{
								...
								_composite.AddMesh(_renderSpace == FilterRenderSpace.Canvas ? null : this.transform, meshInfo.mesh, meshInfo.material, false);
		...
		private void ApplyOutputMeshAndMaterial()
		{
			...
			{
				_screenRect.BuildScreenQuad(null, null, 1f, _quadVertices);
			}
			else
			{
				Camera renderCamera = GetRenderCamera();
				_screenRect.BuildScreenQuad(renderCamera, this.transform, 1f, _quadVertices);
			}
