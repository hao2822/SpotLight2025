...
namespace ChocDino.UIFX
{
	...
	public class UIToolkitTextSource : MaskableGraphic, IMaterialModifier, IMeshModifier
	{
		...
		private void ApplyTextSettings()
		{
			...
			if (_wrap == WhiteSpace.NoWrap || _wrap == WhiteSpace.Pre)
			{
				...
				switch (_textAnchor)
				{
					...
					_textElement.style.left = (size.x - _rectTransform.sizeDelta.x);
					break;
		...
		protected override void OnEnable()
		{
			if (_material == null)
			{
				Shader shader = Shader.Find("Hidden/ChocDino/UIFX/Blend-UIToolkit");
				if (shader)
				{
