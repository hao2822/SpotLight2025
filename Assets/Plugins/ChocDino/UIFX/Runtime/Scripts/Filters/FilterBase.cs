...
namespace ChocDino.UIFX
{
	...
	public abstract partial class FilterBase : UIBehaviour, IMaterialModifier, IMeshModifier
	{
		...
		protected override void OnDisable()
		{
			...
			#if UIFX_FILTER_TMP
			// TODO: For TMP we have to restore the CanvasRenderer mesh+material
			CanvasRenderComponent.SetMesh(null);
			CanvasRenderComponent.materialCount = 1;
			CanvasRenderComponent.SetMaterial(null, 0);
			#endif
		...
		private void BuildOutputQuad(VertexHelper verts)
		{
			...
			{
				_screenRect.BuildScreenQuad(null, null, _lastRenderAlpha, verts);
			}
			else
			{
		...
		protected bool GenerateScreenRect()
		{
			...
			// Calculate screen area
			{
				...
				#if UIFX_BLUR_FILTER_SUPPORT_CHILDREN
				if (_children.Count > 0)
				{
					for (int i = 0; i < _children.Count; i++)
					{
						IFilterChild child = _children[i];
						if (child.IsRenderable())
						{
							_screenRect.AddMeshBounds(_renderSpace == FilterRenderSpace.Canvas ? null : _children[i].GetTransform(), _children[i].GetMesh());
						}
					}
				}
				#endif
				...
				// Expand to accomodate filter
				{
					...
					if (_expand == FilterExpand.Expand)
					{
						...
						if (_renderSpace == FilterRenderSpace.Canvas)
						{
							leftDown.x = Mathf.CeilToInt(leftDown.x / ResolutionScalingFactor);
							leftDown.y = Mathf.CeilToInt(leftDown.y / ResolutionScalingFactor);
							rightUp.x = Mathf.CeilToInt(rightUp.x / ResolutionScalingFactor);
							rightUp.y = Mathf.CeilToInt(rightUp.y / ResolutionScalingFactor);
						}
				...
				if (_composite.Start(_renderSpace == FilterRenderSpace.Canvas ? null : renderCamera, _screenRect.GetTextureRect(), _renderSpace == FilterRenderSpace.Canvas ? _canvas.scaleFactor : 1f))
				{
					...
					if (_children.Count > 0)
					{
						for (int i = 0; i < _children.Count; i++)
						{
							...
							if (child.IsRenderable())
							{
								_composite.AddMesh(_renderSpace == FilterRenderSpace.CanvasSpace ? null : child.GetTransform(), child.GetMesh(), child.GetMaterial());
							}
						}
					}
					#endif
		...
		internal virtual string GetDebugString()
		{
			...
			#if UIFX_FILTER_SUPPORT_CHILDREN
			result += "Children: " + _children.Count;
			#endif
			return result;
		}
