Shader "Hidden/ChocDino/UIFX/Blend-Extrude"
{
	Properties
	{
		...
		_SourceAlpha ("Source Alpha", Float) = 1.0
	...
	v2fExtrude vertExtrude (appdata v)
	{
		v2fExtrude o;

		UNITY_SETUP_INSTANCE_ID(v); // VR SUPPORT
		UNITY_INITIALIZE_OUTPUT(v2fExtrude, o); // VR SUPPORT
	...
	float4 fragExtrude(v2fExtrude i) : SV_Target
	{
		float4 color = 0.0;
		
		float4 shadowColor = tex2D(_ResultTex, i.uv.zw);

//		return float4(abs(shadowColor.xyz), 1.0);
	...
	SubShader
	{
		...
		Pass
		{
			Name "Blend-Extrude"
			CGPROGRAM
			#pragma vertex vertExtrude
			#pragma fragment fragExtrude
			ENDCG
		}
	}
}
